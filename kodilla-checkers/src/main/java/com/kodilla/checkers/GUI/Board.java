...
public class Board {
    ...
    public boolean isEnemyOnEdge(Position unitPosition) {
        return Position.isBoardEdge(unitPosition);
    }
    ...
    private void generateBoard() {
        for (int y = 0; y < Game.BOARD_SIZE; y++) {
            for (int x = 0; x < Game.BOARD_SIZE; x++) {
                ...
                generateTile(new Position(x, y));
            }
        }
    }

    private void generateTile(Position position) {
    ...
    private void spawnUnit(Position position, Group teamUnits, Team team) {
    ...
    private void highlightMove(Move move, PlayerType playerType) {
        Position destination = move.getTarget();
    ...
    private ArrayList<Move> getUnitsPossibleMoves(Unit unit) {
        ...
        for (Position adjacentPositions : unit.getAdjacentPositions()) {
    ...
    private boolean isAttackPossible(Position adjacentTile) {
        return isEnemyUnit(adjacentTile) && !isEnemyOnEdge(adjacentTile) && !isOccupiedTile(adjacentTile.getNextOnPath());
    }

    public boolean isOccupiedTile(Position position) {
        return getTile(position).hasUnit();
    }

    private boolean isEnemyUnit(Position position) {
    ...
    public boolean executeMove(Move move) {
        Position origin = move.getTarget().origin;
        ...
        switch (move.getType()) {
            ...
            case NORMAL:
                ...
                break;
            case KILL:
                Unit attackedUnit = getUnitAtPos(Position.getKillCoords(move));
                moveUnit(origin, target, unit, kingIsCreated);
    ...
    public void moveUnit(Position origin, Position target, Unit unit, boolean kingIsCreated) {
        unit.move(target);
    ...
    private Unit getUnitAtPos(Position position) {
